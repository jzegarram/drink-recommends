...
const Dashboard = () => {
  ...
  const { data: categories, loading: loadingCategories, selectTag: selectCategory, unSelectTag: unSelectCategory } = useCategory(); 

  const { data: ingredients, loading: loadingIngredients, selectTag: selectIngredient, unSelectTag: unSelectIngredient, limitReached: limitReachedIngedients } = useIngredient()
  ...
  const onFilter = () => {
    const filterCategories = categories.filter((category) => category.selected).map((category) => category.strCategory!) as string[];
    const filterIngredients = ingredients.filter((ingredient) => ingredient.selected).map((ingredient) => ingredient.strIngredient1!) as string[];
  ...
  return (
    <Container maxWidth="xl" sx={{background: "linear-gradient(327.21deg, rgba(33, 0, 75, 0.24) 3.65%, rgba(60, 0, 136, 0) 40.32%), linear-gradient(245.93deg, rgba(209, 21, 111, 0.16) 0%, rgba(209, 25, 80, 0) 36.63%), linear-gradient(147.6deg, rgba(58, 19, 255, 0) 29.79%, rgba(98, 19, 255, 0.01) 85.72%), #13111C"}} >
      <Grid container minHeight={"100vh"} >
        {/* <Grid item xs={12} mt={12} mb={6} >
          <Typography color={"#d16fb7"} align='center' fontFamily={'Sedgwick Ave Display'} fontSize={"5rem"} component="h1" fontWeight={"bold"} gutterBottom>
Mix          </Typography>
        </Grid> */}
        <Grid item xs={12} mt={12} mb={6} >
        ...
        <Stack direction="row" spacing={2} useFlexGap flexWrap="wrap" justifyContent={"center"} paddingX={{md: "120px"}}>
          {
      ...
      <Grid item xs={12} mb={12} paddingX={{md: "200px"}}>
