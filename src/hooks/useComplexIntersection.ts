...
export const useComplexIntersection = ({ query }: Props) => {
  ...
  async function fetchData() {
    ...
    try {
      ...
      if (unionBetweenCategories.length === 0 && unionBetweenIngredients.length === 0) {
        setIntersectionData([]);
        return;
      }
      if (unionBetweenCategories.length === 0 && unionBetweenIngredients.length > 0) {
        setIntersectionData(unionBetweenIngredients);
        return;
      }
      if (unionBetweenCategories.length > 0 && unionBetweenIngredients.length === 0) {
        setIntersectionData(unionBetweenCategories);
        return;
      }
